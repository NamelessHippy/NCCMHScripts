...
while(!$clear){
    ...
    try{
        ...
        $directoryRoles=Get-MgRoleManagementDirectoryRoleEligibilitySchedule -Filter "PrincipalId eq '$($user.Id)'"
    }
    catch{
    }
    ...
    if($directoryRoles){
        ...
        foreach($admin in $pAdmin){
            ...
            $body=$null
            try{
                Remove-MgRoleManagementDirectoryRoleEligibilitySchedule -UnifiedRoleAssignmentId $admin.Id -ErrorAction Stop
            }
            catch{
                $_
                Write-Host "Oops, I had an error (,,>`﹏<,,)"
            }
            $sched=Get-MgRoleManagementDirectoryRoleEligibilityScheduleSchedule -Filter "PrincipalId eq '$($user.Id)' and RoleDefinitionId eq '$($admin.Id)'"
            if($sched){
                ...
                #Remove the role.
                try{
                    New-MgRoleManagementDirectoryRoleEligibilityScheduleScheduleRequest -BodyParameter $body -ErrorAction Stop|Out-Null
                }
                catch{
                    $code=@(
        ...
        foreach($role in $activRoles){
            ...
            $body=$null
            try{
                Remove-MgRoleManagementDirectoryRoleEligibilitySchedule -UnifiedRoleAssignmentId $role.Id
            }
            catch{
                $_
                Write-Host "Oops, I had an error (,,>`﹏<,,)"
            }
            $sched=Get-MgRoleManagementDirectoryRoleEligibilityScheduleSchedule -Filter "principal eq '$($user.Id) and roleDefinitionId eq '$($role.roleDefinitionId)"
            if($sched){
                ...
                try{
                    New-MgRoleManagementDirectoryRoleEligibilityScheduleScheduleRequest -BodyParameter $body|Out-Null
                }
                catch{
                    $code=@(
                        $_.Exception.StatusCode
...
Write-Host "I have finished removing all Eligible Assgined Admin Roles from $($user.DisplayName)"
Pause